...
BOOST_AUTO_TEST_CASE( test_integrateMesh ){

    floatVector points = { -1.00000000e+00, -1.00000000e+00,  1.00000000e+00, -6.66666667e-01,
                           -1.00000000e+00,  1.00000000e+00, -3.33333333e-01, -1.00000000e+00,
                            1.00000000e+00, -1.11022302e-16, -1.00000000e+00,  1.00000000e+00,
                            3.33333333e-01, -1.00000000e+00,  1.00000000e+00,  6.66666667e-01,
                           -1.00000000e+00,  1.00000000e+00,  1.00000000e+00, -1.00000000e+00,
    ...
    std::vector< unsigned int > connectivity = {
                                                     0,   2,  16,  14,   1,   9,  15,   7,   8,   2,   4,  18,  16,
                                                     3,  11,  17,   9,  10,   4,   6,  20,  18,   5,  13,  19,  11,
                                                    12,  14,  16,  30,  28,  15,  23,  29,  21,  22,  16,  18,  32,
                                                    30,  17,  25,  31,  23,  24,  18,  20,  34,  32,  19,  27,  33,
                                                    25,  26,  28,  30,  44,  42,  29,  37,  43,  35,  36,  30,  32,
    ...
    floatVector nodalValues( points.size( ) / 3, 1 );

    floatVector answer = { 24. };
    ...
    floatVector spherical_points( points.size( ), 0 );

    floatType radius = 0.47;

    for ( unsigned int i = 0; i < points.size( ) / 3; i++ ){

        floatVector p( points.begin( ) + 3 * i, points.begin( ) + 3 * ( i + 1 ) );

        p /= vectorTools::l2norm( p );

        p *= radius;

        spherical_points[ 3 * i + 0 ] = p[ 0 ];

        spherical_points[ 3 * i + 1 ] = p[ 1 ];

        spherical_points[ 3 * i + 2 ] = p[ 2 ];

    }

    answer = { 2.77417423 };

    BOOST_CHECK( !surfaceIntegration::integrateMesh( spherical_points, connectivity, nodalValues, result ) );

    BOOST_CHECK( vectorTools::fuzzyEquals( answer, result ) );

    std::iota( nodalValues.begin( ), nodalValues.end( ), 0 );
